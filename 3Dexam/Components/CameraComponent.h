...
public:
	...
	//the minimum speed of the camera
	float MinSpeed = 10;

	//the maximum speed of the camera
	float MaxSpeed = 500;

	//the scalar for adjusting the speed of the camera
	float SpeedAdjustmentScalar = 1;
    ...
    explicit CameraComponent(glm::vec3 position = glm::vec3(0, 3, 20), glm::vec3 up = glm::vec3(0, 0.3f, 0), float yaw = YAW, float pitch = PITCH);
    ...
    void ProcessMouseScroll(float InValue);
